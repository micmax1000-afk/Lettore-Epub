...
// === ATTESA CHE EPUB.JS SIA CARICATO ===
function waitForEpub(callback, retries) {
    retries = retries || 0;
    if (typeof ePub !== 'undefined') {
        console.log('✅ EPUB.js trovato!');
        callback();
        return;
    }
    if (retries > 20) {
        console.error('❌ EPUB.js non caricato dopo 20 tentativi');
        document.getElementById('status').textContent = '❌ EPUB.js NON CARICATO! Verifica che il file epub.min.js sia presente';
        document.getElementById('status').className = 'status error';
        return;
    }
    console.log('⏳ Attendo EPUB.js... tentativo ' + (retries + 1));
    setTimeout(function() {
        waitForEpub(callback, retries + 1);
    }, 500);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // === INIZIALIZZAZIONE APP (dopo che EPUB.js è caricato) ===
    function initApp() {
        if (typeof ePub === 'undefined') {
        ...
        setStatus('✅ EPUB.js caricato, pronto', 'ok');
    ...
    // === ATTENDI CHE EPUB.JS SIA CARICATO ===
    waitForEpub(initApp);
});
